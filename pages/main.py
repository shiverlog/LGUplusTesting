...
class TestCase03(Base):
    ...
    # def create_locator(self, main_locator, sub_locator=""):
    #     return (By.CSS_SELECTOR, f"{self.locators[main_locator]}{sub_locator}")

    def execute(self):
        try:
            # device_section으로 포커싱
            device_locator = self.locators['device_section']
            device_section = find_element(self.driver, (By.CSS_SELECTOR, device_locator), f"기기 추천 영역")
            move_to_element(self.driver, device_section, f"기기 추천 영역")
            
            # 탭 리스트 확인
            find_elements(self.driver, (By.CSS_SELECTOR, device_locator + " .tab-wrap ul li a"), f"기기 추천 탭")
        
            # 랜덤 탭 선택
            # select_random_list_one(self.driver, (By.CSS_SELECTOR, device_locator + " .tab-wrap ul li a"), f"기기 추천 탭")
